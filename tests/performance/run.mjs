...
/**
 * Number of times test are runs on each browser and application configuration.
 ...
 *   - the first one testing the current build
 *   - the second one testing the last production version.
 */
const tasks = [];
 ...
 *   - the first one contains the test results of the current build
 *   - the second one contains the test results of the last version
 */
const allSamples = [[], []];
...
async function initServers() {
  ...
  servers.push(staticServer, resultServer);
  await Promise.all([
    staticServer,
...
async function initScripts() {
  await prepareCurrentAppTests();
  await prepareLastAppTests();
}

/**
 * Prepare file(s) for testing the current App.
 * @returns {Promise}
 */
async function prepareCurrentAppTests() {
  try {
    await unlink(path.join(__dirname, "main.current.mjs"));
  } catch (_) {
    // TODO we don't care
  }
  await symlink(
    path.join(__dirname, "..", "..", "main.mjs"),
    path.join(__dirname, "main.current.mjs")
  );
}

/**
 * Prepare file(s) for testing the last version of the App.
 * @returns {Promise}
 */
async function prepareLastAppTests() {
  try {
    await unlink(path.join(__dirname, "main.last.mjs"));
  } catch (_) {
    // TODO we don't care
  }
  await spawnProc(
    ...
    [
      ...
      "master:main.mjs",
      ">",
      path.join(__dirname, "main.last.mjs"),
    ],
...
async function startAllTestsOnChrome() {
  ...
  for (let i = 0; i < TEST_ITERATIONS; i++) {
    tasks.push(() => startCurrentAppTestsOnChrome(i * 2, TEST_ITERATIONS * 2));
    tasks.push(() => startLastAppTestsOnChrome(i * 2 + 1, TEST_ITERATIONS * 2));
  }
  if (CHROME_CMD === null) {
    // eslint-disable-next-line no-console
...
async function _startAllTestsOnFirefox() {
  ...
  for (let i = 0; i < TEST_ITERATIONS; i++) {
    tasks.push(() => startCurrentAppTestsOnFirefox(i * 2, TEST_ITERATIONS * 2));
    tasks.push(() =>
      startLastAppTestsOnFirefox(i * 2 + 1, TEST_ITERATIONS * 2)
    );
  }
  if (FIREFOX_CMD === null) {
    // eslint-disable-next-line no-console
...
/**
 * Start Chrome browser running performance tests on the current App
 * version.
 * @returns {Promise}
 */
async function startCurrentAppTestsOnChrome(testNb, testTotal) {
  // eslint-disable-next-line no-console
  console.log(
    "Running tests on Chrome on the current App version " +
      `(${testNb}/${testTotal})`
  );
...
/**
 * Start Firefox browser running performance tests on the current App
 * version.
 * @returns {Promise}
 */
async function startCurrentAppTestsOnFirefox(testNb, testTotal) {
  // eslint-disable-next-line no-console
  console.log(
    "Running tests on Firefox on the current App version " +
      `(${testNb}/${testTotal})`
  );
...
/**
 * Start Chrome browser running performance tests on the last published App
 * version.
 * @returns {Promise}
 */
async function startLastAppTestsOnChrome(testNb, testTotal) {
  // eslint-disable-next-line no-console
  console.log(
    "Running tests on Chrome on the previous App version " +
      `(${testNb}/${testTotal})`
  );
 ...
 * @returns {Promise}
 */
async function startLastAppTestsOnFirefox(testNb, testTotal) {
  // eslint-disable-next-line no-console
  console.log(
    "Running tests on Firefox on the previous App version " +
      `(${testNb}/${testTotal})`
  );
    ...
    case "win32": {
      ...
      for (const prefix of prefixes) {
        try {
          ...
          await access(windowsChromeDirectory);
